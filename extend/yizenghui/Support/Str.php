...
use \yizenghui\Support\Traits\Macroable;
/**
 * copy from laravel frame
 *
 * remove func plural
 * remove func singular
 * Class Str
...
class Str
{
     ...
     * @return bool|string
     */
    public static function mate($pattern, $value)
    {
        if ($pattern == $value) {
            return false;
        }
        ...
        // 星号表示要解释(匹配出来的)内容
        // 用于获取一段字符串中某个关键词
        // 例如用 "hello,*" 来获取 "hello,my friend" 中的"my friend"
        $pattern = str_replace('\*', '(.*)', $pattern);

        if( preg_match('#^'.$pattern.'\z#u', $value, $matches) && $matches ){
            return end($matches);
        }
